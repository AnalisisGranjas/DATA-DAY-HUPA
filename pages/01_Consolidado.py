...
st.markdown(
    "Visualización y consulta consolidada con métricas avanzadas de **Costo/Huevo**, días de la semana y **personalización de columnas**."
)
st.divider()
...
if df_base is not None and not df_base.empty:
    ...
    # --- FILTROS DE CONSULTA CON HOY - 1 DÍA ---
            ...
            if not fechas_validas.empty:
                ...
                with b_c1:
                    if st.button("7 días", use_container_width=True):
                        st.session_state["rango_fechas"] = [max(min_fecha_dt, max_fecha_dt - timedelta(days=7)), max_fecha_dt]
                        st.rerun()
                with b_c2:
                    if st.button("15 días", use_container_width=True):
                        st.session_state["rango_fechas"] = [max(min_fecha_dt, max_fecha_dt - timedelta(days=15)), max_fecha_dt]
                        st.rerun()
                with b_c3:
                    if st.button("30 días", use_container_width=True):
                        st.session_state["rango_fechas"] = [max(min_fecha_dt, max_fecha_dt - timedelta(days=30)), max_fecha_dt]
                        st.rerun()

                rango_fechas = st.date_input(
                    "Rango de Fechas Seleccionado:",
                    key="rango_fechas",
                    min_value=min_fecha_dt,
                    max_value=max_fecha_dt,
                )
            else:
                rango_fechas = []
    ...
    # --- MAPEO DE COLUMNAS DE RENDERIZADO ---
    ...
    columnas_totales_def = [
        ...
        ("Costo Huevo/Alimento", None, "huevos", "currency_dec"),
    ...
    with st.expander("👁️ **Personalizar Columnas Visibles (Ocultar Costos u Otras)**", expanded=False):
        nombres_todas = [n for n, _, _, _ in columnas_totales_def]
        cols_seleccionadas_nombres = st.multiselect(
            "Selecciona o desmarca las columnas que deseas incluir en el reporte y descarga:",
            options=nombres_todas,
            default=nombres_todas,
            key="multiselect_cols_consolidado",
        )
    ...
    if mostrar_desglose and fechas_unicas_dt:
        fechas_opc = [
            f"{DIAS_ESPANOL.get(pd.to_datetime(f).strftime('%A'), '')} {pd.to_datetime(f).strftime('%d/%m/%Y')}"
            for f in fechas_unicas_dt
        ]
        with col_vis2:
            fecha_a_desglosar = st.selectbox(
                "Selecciona la fecha a desglosar:",
                options=["Todas las Fechas"] + fechas_opc,
                index=0
            )
    ...
    for f_dt in fechas_unicas_dt:
        ...
        for nombre_final, col_orig, _, _ in columnas_totales_def:
            if col_orig and col_orig in df_sub_fecha.columns:
                fila_fecha[nombre_final] = df_sub_fecha[col_orig].sum()

        val_costo = df_sub_fecha[c_costo_alim].sum() if c_costo_alim in df_sub_fecha.columns else 0
        val_prod = df_sub_fecha[c_prod_huevo].sum() if c_prod_huevo in df_sub_fecha.columns else 0
        fila_fecha["Costo Huevo/Alimento"] = (val_costo / val_prod) if val_prod > 0 else 0.0
        ...
        if debe_desglosar_esta:
            ...
            for _, row_gl in df_sub_fecha_agrup.iterrows():
                ...
                for nombre_final, col_orig, _, _ in columnas_totales_def:
                    if col_orig and col_orig in row_gl.index:
                        fila_gl[nombre_final] = row_gl[col_orig]
                
                c_lote_val = row_gl.get(c_costo_alim, 0) if c_costo_alim else 0
                p_lote_val = row_gl.get(c_prod_huevo, 0) if c_prod_huevo else 0
                fila_gl["Costo Huevo/Alimento"] = (c_lote_val / p_lote_val) if p_lote_val > 0 else 0.0
                
                filas_unificadas.append(fila_gl)

    if filas_unificadas and columnas_ordenadas:
        df_unificado = pd.DataFrame(filas_unificadas)

        fila_total_general = {"Fecha / Concepto": "TOTAL GENERAL"}
        for nombre_final, _, _, _ in columnas_totales_def:
            if nombre_final in df_unificado.columns:
                if nombre_final == "Costo Huevo/Alimento":
                    continue
                elif "saldo" in nombre_final.lower():
                    if mostrar_desglose:
                        filas_max_dia = df_unificado[~df_unificado["Fecha / Concepto"].str.contains("└─")]
                    else:
                        filas_max_dia = df_unificado
                    fila_total_general[nombre_final] = filas_max_dia[nombre_final].iloc[-1] if not filas_max_dia.empty else 0
                else:
                    if mostrar_desglose:
        ...
        tot_costo = fila_total_general.get("Costo Alimento", 0)
        tot_prod = fila_total_general.get("Producción Huevos Día", 0)
        fila_total_general["Costo Huevo/Alimento"] = (tot_costo / tot_prod) if tot_prod > 0 else 0.0
        ...
        <style>
            ...
            .custom-table { width: 100%; border-collapse: collapse; font-size: 11px; }
            .custom-table th {
                position: sticky;
                top: 0;
                z-index: 2;
            ...
            .custom-table td { padding: 5px 4px; text-align: right; border: 1px solid #e2e8f0; white-space: nowrap; font-size: 10.5px; }
            .th-fecha { background-color: #f1f5f9; min-width: 140px; text-align: left !important; }
        ...
        <table class="custom-table">
            <thead>
                <tr>
                    <th class="th-fecha">Día & Fecha / Granja</th>
        """
        ...
        buffer = io.BytesIO()
        cols_exportar = ["Fecha / Concepto"] + [n for n, _, _, _ in columnas_ordenadas if n in df_display.columns]
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            df_display[cols_exportar].to_excel(
        ...
        st.warning("Selecciona al menos una columna en el panel desplegable para mostrar.")
    else:
