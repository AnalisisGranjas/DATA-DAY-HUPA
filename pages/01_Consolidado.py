...
st.markdown(
    ...
    .th-fecha { background-color: #f1f5f9; color: #334155; text-align: left !important; min-width: 170px !important; max-width: 220px !important; }
...
st.markdown("Visualización en 3 cuadros de auditoría con **métricas KPI comparativas**, colores por bloque y filtros automatizados.")
st.divider()
...
if df_base is not None and not df_base.empty:
    ...
    # DICCIONARIOS PARA FORMATO LARGO DE FECHA
    DIAS_COMPLETOS_ESPANOL = {
    ...
    def formatear_fecha_larga(f_dt):
        dt = pd.to_datetime(f_dt)
        dia_nom = DIAS_COMPLETOS_ESPANOL.get(dt.strftime("%A"), "")
        mes_nom = MESES_ESPANOL.get(dt.month, "")
        return f"{dia_nom}, {dt.strftime('%d')} {mes_nom} {dt.year}"
    ...
    if not fechas_validas.empty:
        ...
        fecha_ayer = min(max_registrada, datetime.now().date() - timedelta(days=1))
        max_fecha_dt = fecha_ayer
        min_fecha_dt = fechas_validas.min().date()

        default_7_dias = max(min_fecha_dt, fecha_ayer - timedelta(days=7))

        if "rango_c1" not in st.session_state:
            st.session_state["rango_c1"] = [fecha_ayer, fecha_ayer]
        if "rango_c2" not in st.session_state:
            st.session_state["rango_c2"] = [default_7_dias, fecha_ayer]
        if "rango_c3" not in st.session_state:
            st.session_state["rango_c3"] = [fecha_ayer, fecha_ayer]
    ...
    columnas_totales_def = [
        ...
        ("Ingreso B<br>X 40 K", c_ingreso_b, "alimento", "int"),
        ("Consumo B<br>X 40 K", c_consumo_b, "alimento", "int"),
        ("Traslado B<br>X 40 K", c_traslado_b, "alimento", "int"),
        ("Ajuste<br>Alimento", c_ajuste_alim, "alimento", "int"),
        ("Saldo B<br>X 40 K", c_saldo_b, "alimento", "int"),
    ...
    def render_tabla_html(df_input, col_concepto_nombre, css_class_scroll="scroll-table-10", audit_sin_datos=False):
        html_code = f"""
        <div class="titulo-bloques-tabla">
            <span style="color: #1e40af;">🔵 AVES</span> &nbsp;—&nbsp; 
            <span style="color: #92400e;">🟡 ALIMENTO</span> &nbsp;—&nbsp; 
            <span style="color: #065f46;">🟢 HUEVO</span> &nbsp;—&nbsp; 
            <span style="color: #9f1239;">🔴 BANDEJA</span>
        </div>
    ...
    if columnas_ordenadas:
        ...
        # ==========================================
        # SECCIÓN DE MÉTRICAS KPI COMPARATIVAS (TOMADAS DE LA SEGUNDA TABLA / CUADRO 2)
        # ==========================================
        rango_c2_sel = st.session_state.get("rango_c2", [])
        if len(rango_c2_sel) == 2:
            f_start, f_end = rango_c2_sel[0], rango_c2_sel[1]
            ...
            st.markdown("<p style='font-size: 14px; font-weight: bold; margin-bottom: 2px; color: #1e293b;'>📈 Resumen Gerencial y Deltas Comparativos (Basados en el Cuadro 2)</p>", unsafe_allow_html=True)
            st.caption(f"Comparando el periodo seleccionado en el Cuadro 2 ({f_start.strftime('%d/%m')} - {f_end.strftime('%d/%m')}) vs. el periodo anterior equivalente ({f_start_prev.strftime('%d/%m')} - {f_end_prev.strftime('%d/%m')}).")
            ...
            with kpi1:
                st.metric(
                    ...
                    delta_color="inverse",
                    help="Costo de alimento requerido para producir 1 huevo."
                )
            with kpi2:
                st.metric(
                    ...
                    help="Suma total de unidades de huevo recolectadas durante el rango seleccionado."
                )
            with kpi3:
                st.metric(
            ...
            with kpi4:
                st.metric(
                    ...
                    value=f"{int(round(c_cons_cur)):,} bultos",
                    delta=f"{int(round(delta_cons)):,}",
                    help="Bultos de alimento (40 kg) consumidos en el rango seleccionado."
                )
            st.divider()

        # ==========================================
        # CUADRO 1: POR FECHA Y GRANJA (POR DEFECTO: SOLO AYER)
        # ==========================================
        st.markdown("<p style='font-size: 14px; font-weight: bold; margin-bottom: 4px; color: #1e293b;'>1️⃣ Consolidado por Fecha y Granja</p>", unsafe_allow_html=True)
        with st.expander("📅 **Filtro de Fecha - Cuadro 1 (Por Granja)**", expanded=True):
            c1_1, c1_2 = st.columns([1, 2])
            with c1_1:
                st.markdown("**Acceso Rápido:**")
                b1, b2, b3 = st.columns(3)
                with b1:
                    if st.button("Ayer (1 día)", key="b1_c1", use_container_width=True):
                        st.session_state["rango_c1"] = [fecha_ayer, fecha_ayer]
                        st.rerun()
                with b2:
                    if st.button("7 días", key="b7_c1", use_container_width=True):
                        st.session_state["rango_c1"] = [max(min_fecha_dt, fecha_ayer - timedelta(days=7)), fecha_ayer]
                        st.rerun()
                with b3:
                    if st.button("15 días", key="b15_c1", use_container_width=True):
                        st.session_state["rango_c1"] = [max(min_fecha_dt, fecha_ayer - timedelta(days=15)), fecha_ayer]
                        st.rerun()
            with c1_2:
        ...
        if fechas_c1:
            ...
            for f_dt in fechas_c1:
                ...
                fecha_str = formatear_fecha_larga(f_dt)
            ...
            st.html(render_tabla_html(df_c1_disp, "Fecha - Granja", css_class_scroll="scroll-table-10", audit_sin_datos=True))
        else:
        ...
        # ==========================================
        # CUADRO 2: CONSOLIDADO GENERAL POR FECHA (POR DEFECTO: ÚLTIMOS 7 DÍAS)
        # ==========================================
        st.markdown("<p style='font-size: 14px; font-weight: bold; margin-bottom: 4px; color: #1e293b;'>2️⃣ Consolidado General por Fecha</p>", unsafe_allow_html=True)
        with st.expander("📅 **Filtro de Fecha - Cuadro 2 (General)**", expanded=True):
            ...
            with c2_1:
                ...
                with b1:
                    if st.button("7 días", key="b7_c2", use_container_width=True):
                        st.session_state["rango_c2"] = [max(min_fecha_dt, fecha_ayer - timedelta(days=7)), fecha_ayer]
                        st.rerun()
                with b2:
                    if st.button("15 días", key="b15_c2", use_container_width=True):
                        st.session_state["rango_c2"] = [max(min_fecha_dt, fecha_ayer - timedelta(days=15)), fecha_ayer]
                        st.rerun()
                with b3:
                    if st.button("30 días", key="b30_c2", use_container_width=True):
                        st.session_state["rango_c2"] = [max(min_fecha_dt, fecha_ayer - timedelta(days=30)), fecha_ayer]
                        st.rerun()
            with c2_2:
        ...
        if fechas_c2:
            ...
            st.html(render_tabla_html(df_c2_disp, "Fecha", css_class_scroll="scroll-table-7"))
        else:
        ...
        # ==========================================
        # CUADRO 3: POR FECHA, GRANJA Y LOTE (POR DEFECTO: SOLO AYER)
        # ==========================================
        ...
        with st.expander("📅 **Filtro de Fecha - Cuadro 3 (Por Granja & Lote)**", expanded=True):
            ...
            with c3_1:
                ...
                b1, b2, b3 = st.columns(3)
                with b1:
                    if st.button("Ayer (1 día)", key="b1_c3", use_container_width=True):
                        st.session_state["rango_c3"] = [fecha_ayer, fecha_ayer]
                        st.rerun()
                with b2:
                    if st.button("7 días", key="b7_c3", use_container_width=True):
                        st.session_state["rango_c3"] = [max(min_fecha_dt, fecha_ayer - timedelta(days=7)), fecha_ayer]
                        st.rerun()
                with b3:
                    if st.button("15 días", key="b15_c3", use_container_width=True):
                        st.session_state["rango_c3"] = [max(min_fecha_dt, fecha_ayer - timedelta(days=15)), fecha_ayer]
                        st.rerun()
            with c3_2:
        ...
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            ...
            if "df_c1_disp" in locals():
                ...
                limpiar_cols(df_c1_disp[cols_e1]).to_excel(writer, index=False, sheet_name="Por_Granja")
            if "df_c2_disp" in locals():
                cols_e2 = ["Fecha / Concepto"] + [n for n, _, _, _ in columnas_ordenadas if n in df_c2_disp.columns]
                limpiar_cols(df_c2_disp[cols_e2]).to_excel(writer, index=False, sheet_name="General_Fecha")
