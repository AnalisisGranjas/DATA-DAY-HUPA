...
st.markdown(
    "Audita la evolución detallada día a día con la estructura completa de **23 columnas de datos, costos por huevo y facturas**."
)
st.divider()
...
if df_base is not None and not df_base.empty:
    ...
    # ESTRUCTURA COMPLETA DE 23 COLUMNAS (INCLUYE COSTO HUEVO/ALIMENTO)
    ...
    if not df_lote_filtrado.empty and columnas_ordenadas:
        ...
        # FECHA CON DÍA DE LA SEMANA (Ej: Vie 04/09/2026)
        ...
        val_costo = df_diario["Costo Alimento"] if "Costo Alimento" in df_diario.columns else 0
        val_prod = df_diario["Producción Huevos Día"] if "Producción Huevos Día" in df_diario.columns else 0
        df_diario["Costo Huevo/Alimento"] = (val_costo / val_prod).fillna(0).replace([float('inf'), -float('inf')], 0)

        # Fila TOTALES
        fila_totales = {"Fecha / Concepto": "TOTALES / ACUMULADO"}
        for nombre_final, _, _, tipo_dato in columnas_totales_def:
            if nombre_final in df_diario.columns:
                if tipo_dato == "text":
                    fila_totales[nombre_final] = "-"
                elif nombre_final == "Costo Huevo/Alimento":
                    continue
        ...
        tot_costo = fila_totales.get("Costo Alimento", 0)
        tot_prod = fila_totales.get("Producción Huevos Día", 0)
        fila_totales["Costo Huevo/Alimento"] = (tot_costo / tot_prod) if tot_prod > 0 else 0.0
        ...
        <style>
            ...
            .custom-table td { padding: 5px 4px; border: 1px solid #e2e8f0; white-space: nowrap; font-size: 10.5px; }
